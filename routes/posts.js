...
router.get('/', function (req, res, next) {
  ...
  Promise.all([
    ...
    .then(function (result) {
      var posts = result[0]
      var total = result[1]
      var totalpage = parseInt(total / constant.PAGE_SIZE)
      if (total % 5 !== 0) {
    ...
    .then(function (result) {
      ...
      if (!post) {
        ...
        var hasPower =
          !post.isPrivate ||
          (isLogin &&
            req.session.user._id.toString() === post.author._id.toString())
        hasPower
          ? res.render('post', {
              post: post,
...
router.post('/:postId/comment', checkLogin, function (req, res, next) {
  ...
  Promise.all([PostModel.getPostById(postId), CommentModel.create(comment)])
    .then(function (result) {
      // 给文章作者发送邮件
...
router.get(
  '/:postId/comment/:commentId/remove',
  checkLogin,
